...
const commentTextInput = document.querySelector(".new-comment-input input");
const addReactionButton = document.querySelector(".add-reaction");
const commentEmojisInput = document.querySelector(".emoji.small-btn");
commentEmojisInput.append(createEmojiPicker(".emoji.small-btn"));
const submitComment = document.querySelector(".new-comment .submit-comment");
const commentsEmptyBody = document.querySelector(".body-empty");
const deleteAlertModal = document.querySelector(".delete-alert");
const deleteBackdrop = document.querySelector(".delete-alert-backdrop");
const deleteAlertCloseBtn = document.querySelector(
  ".delete-alert .btn-close"
  );
const cancelDeletionBtn = document.querySelector(".delete-alert .cancel");
const confirmDeletionBtn = document.querySelector(
  ".modal-footer .submit-btn"
);
// Consts

// Functions
function createEmojiPicker(insert) {
  // Emoji container
  const fgEmojiContainer = document.createElement("div");
  fgEmojiContainer.classList.add("fg-emoji-container");
  if (insert == ".add-reaction") {
    fgEmojiContainer.classList.add("for-reactions");
  } else {
    fgEmojiContainer.classList.add("for-comments");
  }
  // Emoji container

  // Categories Navbar
  const emojiNav = document.createElement("nav");
  emojiNav.classList.add("fg-emoji-nav");
  fgEmojiContainer.append(emojiNav);
  const categoriesNav = document.createElement("ul");
  emojiNav.append(categoriesNav);
  Object.entries(categoryFlags).forEach((cate) => {
    let li = document.createElement("li");
    let a = document.createElement("a");
    a.setAttribute("title", cate[0]);
    a.setAttribute("href", `#${cate[0]}`);
    a.innerHTML = cate[1];
    a.addEventListener("click", (e) => {
      e.preventDefault();
      let title = e.currentTarget["title"];
      let sectionGoal = document.querySelector(`div#${title}`);
      sectionGoal.scrollIntoView({ block: "start", behavior: "smooth" });
    });
  ...
  closeBtnLi.classList.add("fg-picker-special-buttons");
  const closeBtnA = document.createElement("a");
  closeBtnA.classList.add("fg-emoji-picker-close-button");
  closeBtnA.innerHTML = closeIcon;

  closeBtnLi.append(closeBtnA);
  categoriesNav.append(closeBtnLi);
  // Categories Navbar

  // Emoji picker body
  const emojiPickerBody = document.createElement("div");
  const bodyUl = document.createElement("ul");
  bodyUl.classList.add("fg-emoji-list");
  emojiPickerBody.append(bodyUl);

  for (let i = 0; i < Object.keys(categoryFlags).length; i++) {
    const categoryWrapper = document.createElement("div");
    categoryWrapper.classList.add("fg-emoji-picker-category-wrapper");
    categoryWrapper.id = Object.keys(categoryFlags)[i];
    const categoryName = document.createElement("p");
    categoryName.textContent = Object.keys(categoryFlags)[i];
    categoryName.classList.add("fg-emoji-picker-category-title");
    categoryWrapper.append(categoryName);
    for (let j = 0; j < Object.values(emojiObj)[i].length; j++) {
      let emojiLi = document.createElement("li");
      let emojiA = document.createElement("a");
      emojiA.setAttribute("href", "#");
      emojiA.textContent = Object.values(emojiObj)[i][j]["emoji"];
      if (insert == ".add-reaction") {
        emojiA.addEventListener("click", (e) => {
          e.preventDefault();
          let data = {
          ...
          let postReactionRequest = fetch("http://127.0.0.1:8000/reactions/", {
            method: "POST",
            headers: {
          ...
          postReactionRequest.then(async (response) => {
            let reactionData = await response.json();
            addCommentReaction(reactionData["comment"], reactionData["emoji"]);
          });
        });
      }
      if (insert == ".emoji.small-btn") {
        emojiA.addEventListener("click", (e) => {
          e.preventDefault();
          let commentTextInput = document.querySelector(
            ".new-comment-input input"
          );
          commentTextInput.value += e.target.textContent;
        });
      }

      emojiLi.append(emojiA);
      categoryWrapper.append(emojiLi);
    }
    bodyUl.append(categoryWrapper);
  }
  fgEmojiContainer.append(emojiPickerBody);
  return fgEmojiContainer;
}

function createReaction(emojiValue) {
  ...
  reactContainer.append(emoji);
  reactContainer.append(count);
  return reactContainer;
}

function addCommentReaction(commentId, emojiValue) {
  const reactionsContainer = document.querySelector(
    `.comment-container[comment-id="${commentId}"] .comment-reactions`
  );
  const react = createReaction(emojiValue);
  reactionsContainer.append(react);
  react.children[1].textContent = 1;
}

async function toggleCommentEmptyBody(state) {
  if (state == "hide") {
    commentsEmptyBody.removeAttribute("state");
    commentsEmptyBody.style.display = "none";
  } else if (state == "check") {
    if (!commentsBody.childElementCount) {
      commentsEmptyBody.removeAttribute("style");
    }
  } else {
    commentsEmptyBody.removeAttribute("style");
  }
...
function createComment(data) {
  ...
  let comment = `<div class="comment-container" comment-id="${data["id"]}">
  <div class="profile-img"><span>${
    document.querySelector(".profile-btn span").textContent
  }</span></div>
  <div class="comment-info">
    <p class="username">${
      document.querySelector(".user p:nth-child(1)").textContent
    }</p>
    <p class="comment-date">${data["posted_date"]}</p>
    <div class="comment-btns"><button class="add-reaction small-btn"><span class="add-reaction-span"><svg
            class="add-reaction-svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
            xmlns="http://www.w3.org/2000/svg">
            <path class="add-reaction-path" fill-rule="evenodd" clip-rule="evenodd"
              d="M20 1.5a.5.5 0 00-1 0V4h-2.5a.5.5 0 000 1H19v2.5a.5.5 0 001 0V5h2.5a.5.5 0 000-1H20V1.5zM12 5a7 7 0 107 7 .5.5 0 011 0 8 8 0 11-8-8 .5.5 0 010 1zm-2.5 6a1 1 0 100-2 1 1 0 000 2zm6-1a1 1 0 11-2 0 1 1 0 012 0zm-6.49 3.6a.502.502 0 01.499-.6h4.982c.322 0 .563.29.499.6-.287 1.388-1.533 2.4-2.99 2.4-1.457 0-2.703-1.012-2.99-2.4z"
              fill="currentColor"></path>
          </svg></span></button>
          <div class="dropdown comment-options">
          <button class="small-btn" data-bs-toggle="dropdown" aria-expanded="false"><svg xmlns="http://www.w3.org/2000/svg"
          width="24" height="24">
        ...
        <li class="dropdown-item"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" aria-hidden="true"><g fill="none" fill-rule="evenodd"><path d="M0 0h24v24H0z"></path><rect width="14" height="1" x="5" y="6" fill="currentColor" rx=".5"></rect><path fill="currentColor" d="M10 9h1v8h-1V9zm3 0h1v8h-1V9z"></path><path stroke="currentColor" d="M17.5 6.5h-11V18A1.5 1.5 0 0 0 8 19.5h8a1.5 1.5 0 0 0 1.5-1.5V6.5zm-9 0h7V5A1.5 1.5 0 0 0 14 3.5h-4A1.5 1.5 0 0 0 8.5 5v1.5z"></path></g></svg> Delete comment</li>
        </ul>
        </div>
        </div>
  </div>
  ...
  `;
  comment = parser.parseFromString(comment, "text/html");
  let addReaction = comment.querySelector(".add-reaction");
  const emojiPicker = createEmojiPicker(".add-reaction");
  let deleteCommentBtn = comment.querySelector(".dropdown-menu .dropdown-item");
  deleteCommentBtn.addEventListener("click", showDeleteAlert);
  addReaction.append(emojiPicker);
  return comment.body.children[0];
}

function addComment(comment) {
  commentsBody.append(comment);
}

function removeComments() {
  let comments = Object.values(commentsBody.children);
  comments.forEach((comment) => {
    commentsBody.removeChild(comment);
  });
}

async function postComment() {
  let data = {
    text: document.querySelector(`.new-comment-input input[type="text"]`).value,
  };
  let postRequest = await fetch("http://127.0.0.1:8000/comments/", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `JWT ${localStorage.getItem("todoist_access_token")}`,
    },
    body: JSON.stringify(data),
  });
  let response = await postRequest.json();
  addComment(createComment(response));
  clearCommentInput();
  toggleCommentEmptyBody("hide");
}

function showDeleteAlert(e) {
  let DELETE_ALERT_MODAL = document.querySelector(".delete-alert");
  let DELETE_BACKDROP = document.querySelector(".delete-alert-backdrop");
  DELETE_ALERT_MODAL.classList.add("show");
  DELETE_BACKDROP.classList.add("show");
  let commentId = e.target
    .closest(".comment-container")
    .getAttribute("comment-id");
  DELETE_ALERT_MODAL.setAttribute("comment-id", commentId);
}

function cancelDeletion() {
  let DELETE_ALERT_MODAL = document.querySelector(".delete-alert");
  let DELETE_BACKDROP = document.querySelector(".delete-alert-backdrop");
  DELETE_ALERT_MODAL.classList.remove("show");
  DELETE_BACKDROP.classList.remove("show");
}

async function deleteComment() {
  let commentId = deleteAlertModal.getAttribute("comment-id");
  let deleteCommentRequest = await fetch(
  ...
  if (deleteCommentRequest.ok) {
    deleteAlertModal.classList.remove("show");
    deleteBackdrop.classList.remove("show");
    let comment = document.querySelector(
      `.comment-container[comment-id="${commentId}"]`
    );
    commentsBody.removeChild(comment);
    toggleCommentEmptyBody("check");
  }
}

async function fetchComments() {
  if (commentsBtn.getAttribute("fetched") == "false") {
    let fetchCommentsRequest = await fetch("http://127.0.0.1:8000/comments/", {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        Authorization: `JWT ${localStorage.getItem("todoist_access_token")}`,
    ...
    let comments = await fetchCommentsRequest.json();
    if (Object.keys(comments).length > 0) {
      comments.forEach((comment) => {
        addComment(createComment(comment));
        comment["reactions"].forEach((reaction) => {
          addCommentReaction(reaction["comment"], reaction["emoji"]);
        });
      });
    } else {
      toggleCommentEmptyBody("show");
    }
    commentsBtn.setAttribute("fetched", "true");
  }
}
...
// Events
commentsBtn.addEventListener("click", fetchComments);

commentsTab.onclick = () => {
  if (commentsTab.classList.contains("active-tab") == false) {
    commentsTab.classList.add("active-tab");
    activityTab.classList.remove("active-tab");
    let commentsSection = document.querySelector(".comments-section");
    let activitySection = document.querySelector(".activity-section");
    commentsSection.style.display = "unset";
...
activityTab.onclick = () => {
  if (activityTab.classList.contains("active-tab") == false) {
    activityTab.classList.add("active-tab");
    commentsTab.classList.remove("active-tab");
    let commentsSection = document.querySelector(".comments-section");
    let activitySection = document.querySelector(".activity-section");
    commentsSection.style.display = "none";
    activitySection.style.display = "unset";
    let title = document.querySelectorAll(".title");
...
submitComment.addEventListener("mousedown", (e) => {
  submitComment.style.scale = "0.95";
});

submitComment.addEventListener("mouseup", (e) => {
  submitComment.style.scale = "1";
});
...
deleteAlertCloseBtn.addEventListener("click", cancelDeletion);

confirmDeletionBtn.addEventListener("click", deleteComment);
// Events
